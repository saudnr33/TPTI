...
def image_align(src_file, dst_file, face_landmarks, output_size=256, transform_size=1024, enable_padding=True):
        ...
        quad = np.stack([c - x - y, c - x + y, c + x + y, c + x - y]) #bottom left, top left, bottom right, top right
        ...
        #Add extra space: here I will extend the bottom right node

        x_vec = quad[3] - quad[0] #top left, bottom left, bottom right, top right
        y_vec = quad[1] - quad[0] 

        x_vec /= np.hypot(*x_vec)
        y_vec /= np.hypot(*y_vec)
        

        extraSpaceFactor = 100
        quad[1] += y_vec * extraSpaceFactor
        quad[2] += y_vec * extraSpaceFactor + x_vec * extraSpaceFactor
        quad[3] += x_vec * extraSpaceFactor
...
if __name__ == '__main__':
    ...
    for img_name in os.listdir(args.src):
        ...
        for i, face_landmarks in enumerate(landmarks_detector.get_landmarks(raw_img_path), start=1):
            ...
            image_align(raw_img_path, aligned_face_path, face_landmarks, args.output_size, args.transform_size, True)